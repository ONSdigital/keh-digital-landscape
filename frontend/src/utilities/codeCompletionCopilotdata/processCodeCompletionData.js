...
export function processCodeCompletionData(data) {
    ...
    */
    ...
    for (const index in data) {
        ...
        let codeCompletionData = day.totals_by_feature?.find(item => item.feature === "code_completion")
        ...
        if (dayLOCSuggested > 0) {numberLOCSuggestions += 1}
        if (dayLOCAccepted > 0) {numberLOCAcceptances += 1}

        // Suggested Graph data
        let suggestionsVSAcceptances = [daySuggested, dayAccepted, (dayAccepted / daySuggested) * 100];
        ...
        let suggestionsLOCVSAcceptancesLOC = [dayLOCSuggested, dayLOCAccepted, (dayLOCAccepted / dayLOCSuggested) * 100];
        ...
        let averageSuggestionsVSAcceptancesLOC = [(dayLOCSuggested / daySuggested), (dayLOCAccepted / dayAccepted)]
        ...
        for (const pieChartLanguage of completionLangRows) {
            ...
            langSuggestionCounts[lang] = (langSuggestionCounts[lang] ?? 0) + (pieChartLanguage.code_generation_activity_count ?? 0);
            langAcceptanceCounts[lang] = (langAcceptanceCounts[lang] ?? 0) + (pieChartLanguage.code_acceptance_activity_count ?? 0);
        }
    }
    
    for (const [lang, count] of Object.entries(langSuggestionCounts)) {
        languagesUsedPieChart.suggestions.push({ [lang]: count / totalGeneratedSuggestions });
        languagesUsedPieChart.acceptances.push({ [lang]: (langAcceptanceCounts[lang] ?? 0) / totalGeneratedSuggestions });
    }

    suggestedCards.suggestions.totalSuggestions = suggestionsSum
    suggestedCards.suggestions.totalAcceptances = acceptancesSum
    suggestedCards.suggestions.acceptanceRate = (acceptancesSum / suggestionsSum) * 100

    suggestedCards.loc.totalLOCSuggestions = suggestionsLOCSum
    suggestedCards.loc.totalLOCAcceptances = acceptancesLOCSum
    suggestedCards.loc.acceptanceLOCRate = (acceptancesLOCSum / suggestionsLOCSum) * 100

    suggestedCards.average.averageLOCSuggestions = suggestionsLOCSum / numberLOCSuggestions
    suggestedCards.average.averageLOCAccepted = acceptancesLOCSum / numberLOCAcceptances

    codeCompletionMetrics = {"suggestedCards": suggestedCards, "suggestedGraph": suggestedGraph, "suggestedLOCGraph": suggestedLOCGraph, "averageSuggestedLOCGraph": averageSuggestedLOCGraph, "languagesUsedPieChart": languagesUsedPieChart}
