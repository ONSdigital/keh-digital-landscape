...
function LegacyDataVisualisation({ data, isLoading }) {
    ...
    for (const index in rawFebData) {
        ...
        febDataMonthly.suggestions.push({ date: day.day, count: day.total_suggestions_count || 0 });
        febDataMonthly.acceptances.push({ date: day.day, count: day.total_acceptances_count || 0 });
        febDataMonthly.linesSuggested.push({ date: day.day, count: day.total_lines_suggested || 0 });
        febDataMonthly.linesAccepted.push({ date: day.day, count: day.total_lines_accepted || 0 });
        febDataMonthly.chats.push({ date: day.day, count: day.total_chat_turns || 0 });
        febDataMonthly.chatAcceptances.push({ date: day.day, count: day.total_chat_acceptances || 0 });
    }
    ...
    for (const index in rawMarData) {
        ...
        (ideCompletions.editors || []).forEach(editor => {
            (editor.models || []).forEach(model => {
                (model.languages || []).forEach(language => {
                    ...
                    marDataTotals.linesSuggested += language.total_code_lines_suggested || 0;
                    marDataTotals.linesAccepted += language.total_code_lines_accepted || 0;
        ...
        marDataMonthly.suggestions.push({ date: day.date, count: dayTotals.suggestions });
        marDataMonthly.acceptances.push({ date: day.date, count: dayTotals.acceptances });
        marDataMonthly.linesSuggested.push({ date: day.date, count: dayTotals.linesSuggested });
        marDataMonthly.linesAccepted.push({ date: day.date, count: dayTotals.linesAccepted });
        marDataMonthly.chats.push({ date: day.date, count: dayTotals.chats });
        marDataMonthly.chatInsertions.push({ date: day.date, count: dayTotals.chatInsertions });
        marDataMonthly.chatCopies.push({ date: day.date, count: dayTotals.chatCopies });
    };

    // Get the start and end dates for both datasets
    const febStartDate = febDataMonthly.suggestions.length > 0 ? febDataMonthly.suggestions[0].date : null;
    const febEndDate = febDataMonthly.suggestions.length > 0 ? febDataMonthly.suggestions[febDataMonthly.suggestions.length - 1].date : null;
    const marStartDate = marDataMonthly.suggestions.length > 0 ? marDataMonthly.suggestions[0].date : null;
    const marEndDate = marDataMonthly.suggestions.length > 0 ? marDataMonthly.suggestions[marDataMonthly.suggestions.length - 1].date : null;
    ...
    return (
        <div>
            <h2>Legacy Copilot Data</h2>
            <p>
                This section visualises the legacy Copilot data for previous revisions of the dashboard. 
                For each legacy dataset, the GitHub API endpoints were different, so the available metrics vary and may not be directly comparable.
                Each dataset is visualised separately, with the available metrics for that dataset shown in the cards and graphs.
            </p>

            <div>
                <h3>{formattedMarStartDate} - {formattedMarEndDate}</h3>
                ...
                <div className="copilot-chat-grid">
                    ...
                    <div className="stat-card" key="mar-acceptance-rate">
                        ...
                        <p>{getPercentage((marDataTotals.acceptances / marDataTotals.suggestions))}</p>
                    </div>
                </div>
                <div className="copilot-chat-grid">
                    ...
                    <div className="stat-card" key="mar-line-acceptance-rate">
                        ...
                        <p>{getPercentage((marDataTotals.linesAccepted / marDataTotals.linesSuggested))}</p>
                    </div>
                </div>
                ...
                <div className="copilot-chat-grid">
                    <div className="stat-card" key="mar-chat-insertion-rate">
                        ...
                        <p>{getPercentage((marDataTotals.chatInsertions / marDataTotals.chats))}</p>
                    </div>
                    <div className="stat-card" key="mar-chat-copy-rate">
                        <h2>Chat Copy Rate</h2>
                        <p>{getPercentage((marDataTotals.chatCopies / marDataTotals.chats))}</p>
                    </div>
                </div>
            ...
            <div>
                <h3>{formattedFebStartDate} - {formattedFebEndDate}</h3>
                ...
                <div className="copilot-chat-grid">
                    ...
                    <div className="stat-card" key="feb-acceptance-rate">
                        <h2>Acceptance Rate</h2>
                        <p>{getPercentage((febDataTotals.acceptances / febDataTotals.suggestions))}</p>
                    </div>
                </div>
                <div className="copilot-chat-grid">
                    ...
                    <div className="stat-card" key="feb-line-acceptance-rate">
                        <h2>Line Acceptance Rate</h2>
                        <p>{getPercentage((febDataTotals.linesAccepted / febDataTotals.linesSuggested))}</p>
                    </div>
                </div>
                ...
                <div className="copilot-chat-grid">
                    ...
                    <div className="stat-card" key="feb-chat-acceptance-rate">
                        ...
                        <p>{getPercentage((febDataTotals.chatAcceptances / febDataTotals.chats))}</p>
                    </div>
                </div>
