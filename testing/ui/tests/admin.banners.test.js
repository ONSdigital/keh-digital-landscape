    ...
    await test.step('Mock backend API calls', async () => {
        ...
        await page.route('**/admin/api/banners/toggle', async route => {
            ...
            route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ message: 'Banner visibility updated successfully' })
            });
        });
 ...
 */
async function fillInBannerForm(page, formValues = { title, message, type, pages }) {
    ...
    for (const pageOption of formValues.pages) {
        await page.locator(`.multi-select-option:has-text("${pageOption}")`).click();
    }
...
test.describe('add banner', () => {
    ...
    for (const type of bannerTypes) {
        test(`should add a ${type} banner and get shown in existing banners list`, async ({ page }) => {
            const bannerData = {
            ...
            const bannerTitleLocator = page.locator(`#banner-title-${newBannerIndex}`);
            const bannerMessageLocator = page.locator(`#banner-message-${newBannerIndex}`);
            const bannerTypeLocator = page.locator(`#banner-type-${newBannerIndex}`);
            const bannerPagesLocator = page.locator(`#banner-pages-${newBannerIndex}`);

            await expect(bannerTitleLocator).toHaveText(`Test ${type} Banner`);
            await expect(bannerMessageLocator).toHaveText(`This is a test ${type} banner message.`);
    ...
    for (const option of pageOptions) {
        test(`should allow selecting ${option.value} page for banner`, async ({ page }) => {
            const bannerData = {
    ...
    test('should allow save to be cancelled in confirmation modal', async ({ page }) => {
        const bannerData = {
    ...
    test('save button should be disabled until all required fields are filled', async ({ page }) => {
        const bannerTitle = page.locator('#banner-title-input');
        const bannerMessage = page.locator('#banner-message-input');
...
test.describe('banner actions', () => {
    test('hide button should hide banner on specified pages', async ({ page }) => {
    ...
    test('delete button should remove banner from specified pages', async ({ page }) => {
        // First, add a banner that is shown on the radar page
        const bannerData = {
    ...
    test('deleting a banner should persist the remaining banners', async ({ page }) => {
        // Add two banners
        const bannerData1 = {
            title: 'Banner 1',
